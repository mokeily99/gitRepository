	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var ix=0; ix<pMenu.length; ix++){
				var p_menu = {"title": pMenu[ix].menuName, "icon": pMenu[ix].icon, "href": webpath+pMenu[ix].menuUrl, "spread": pMenu[ix].isShow};
				p_menu.children = [];
				for(var iy=0; iy<sMenu.length; iy++){
					if(sMenu[iy].pid == pMenu[ix].maxaccept){
						var children = {"title": sMenu[iy].menuName, "icon": (sMenu[iy].icon=="")?" ":sMenu[iy].icon, "href": webpath+sMenu[iy].menuUrl, "spread": sMenu[iy].isShow};
						p_menu.children.push(children);
					}
				}
				nav_menu.push(p_menu);
			}
		}
	});
	return nav_menu;
